...
#' @param decreasing logical indicating if a monotonic decreasing should be fit (instead of monotonic increasing)
...
#'
#' @param X0 Matrix of samples (rows) from 0  distribution
#' @param Xm Matrix of samples (rows) from mixture distribution
#' @return lower bound for the mixing coefficient alpha
#' @examples
...
compute_Patra_bound <- function (X0, Xm){
  ...
  if(!is.matrix(Xm)) Xm <- as.matrix(Xm)
